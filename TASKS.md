...
- depends-on: Part 4
- exit: the app opens on **naked walls** — structure only, no finish; one checkbox per service layer adds water, gas, electricity, low voltage and climate over it; the **last checkbox is `texture`**, which is the finished look v1.0.0 already ships, so ticking everything reproduces today's app pixel for pixel. Every run is declared once in `plan.ts`, checked by `pnpm verify:plan`, terminates in one of the control center's two isolated compartments, and reaches the rest of the floor through the side-B strip
...
- [ ] `plan.ts` — `SERVICE_RUNS`, declared like `PORTS`, `WINDOWS` and `FIXTURES` are: layer, the two ends, the polyline on the centimetre grid, the height band it runs at, and its bore. Matricule tag **`S`** — `W`, `P`, `G` and `X` are taken by walls, ports, glazing and fixtures, `R` by the rooms, and `S` is free: a room's _type_ code is three letters (`STR`, `CTR`), so `F1-R08-CTR-S1` reads unambiguously and no existing matricule can be confused with it. (If `S` still reads too close to "stair" to the owner, `U` is the fallback; the letter is a naming decision and belongs in the ADR, not in whichever module needs it first)
