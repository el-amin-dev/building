...
- [ ] ADR: services as a declared layer of the plan — the matricule tag, the run schema, the layer order, naked walls as the default view, and the two compartments. Proposed **ADR-022**; it closes the deferral brief §7.4 has carried since plan v2 and the `open` note R08 carries in `plan.ts`
- [ ] `plan.ts` — `SERVICE_LAYERS`: the ordered layer list, which is both the build order and the order the checkboxes are listed in. `water` · `gas` · `electricity` · `lowVoltage` (ethernet and the rest of the low-voltage side) · `climate` (the cooling and heating ducts) — and `texture`, which is not a service but the finished look, so it is the last one and is marked as such rather than being one more service in the list
- [ ] `plan.ts` — `SERVICE_RUNS`, declared like `PORTS`, `WINDOWS` and `FIXTURES` are: layer, the two ends, the polyline on the centimetre grid, the height band it runs at, and its bore. Matricule tag **`S`** — `W`, `P`, `G` and `X` are taken by walls, ports, glazing and fixtures and `R` by the rooms. **Built as `F1-DRN-S4`**: floor, a three-letter LAYER code (`DRN` `WTR` `GAS` `ELE` `ELV` `CLM`), then the `S` number, numbered within its layer in declaration order. Not the room-scoped `F1-R08-CTR-S1` this bullet first sketched — a run crosses rooms, so scoping its number to one of them would have named it after wherever it happened to start, and a single global sequence would have renumbered the whole installation the day a bathroom branch was added (ADR-022)
- [ ] `plan.ts` — `SERVICE_SPEC`: the bore, the drop and the clearance of each layer, in one place, the way `FIXTURE_SPEC` holds the fixture sizes. No run writes its own diameter, so changing a pipe size moves every pipe of that layer together
- [ ] `plan.ts` — **pay off the deferral**: the control center becomes two isolated compartments, gas and water in one, electricity and the low-voltage side in the other (brief §7.4). Delete the `open` note on R08 rather than editing it, and say in the ADR which of the two the `servicesCabinet` becomes — or whether it becomes two cabinets, which is what "one box so there is a single object to divide" was written for
- [ ] `domain/services.ts` — the runs as boxes, the way `domain/fixtures.ts` builds the fixtures: footprints stay in `plan.ts` so `verify:plan` gates them, elevations live in the domain module, and every vertical level comes from the `heights` argument so nothing is written down twice (ADR-015 is the precedent and the pattern to copy)
...
- [ ] `isServicedSpace` is **not** to be taught about runs. A run crossing a room does not make that room serviced: `SERVICING_ROLES` (`fitting` · `appliance` · `services`) answers a question about what stands _in_ a room, and a pipe in the ceiling void of a bedroom would silently lay marble in it. ADR-021 fixed exactly this class of defect once; a test pinning the floor finishes across the whole floor before and after this part is the cheap guard
...
- [ ] `application/layerStore.ts` — the layer state, following the house rule the other stores already keep (a no-op update returns the previous state, `floorCountStore.ts` is the model). The default is naked walls with every box unticked
- [ ] `ui/LayerSwitcher.tsx` — a real `<fieldset>` with a `<legend>`, one native `<input type="checkbox">` per layer with its own accessible name, visible focus, AA contrast for both the box and its label, and a hit target of at least 24 × 24 CSS px at the 400 px width the HUD is already measured at. Native checkboxes, not styled `div`s: the HUD's WCAG 2.2 AA record is the thing Part 3 spent a part earning
- [ ] **Pointer-operable, not keyboard-only.** The owner cannot always use a keyboard, and every key binding in this app already has an on-screen hold-to-act equivalent (`RemoteControl.tsx`, ADR-008/ADR-013). A checkbox is pointer-operable by nature, so the rule bites the other way here: if the switcher gets a keyboard shortcut, that shortcut is an addition to the on-screen control and never a replacement for it
...
- [ ] `ui/RoomInfoPanel.tsx` — the room panel names the services that reach the room, derived from the runs the way daylight is derived from four values rather than from a list of lit rooms (Part 4). A hand-kept list of which room has water in it is a second copy of the runs
