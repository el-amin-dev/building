...
- "Go to room" (interior view only): a disclosure button listing every reachable room by the same `R11/KIT · Kitchen` label, in matricule order; picking one **walks** you there through the real doorways at normal walking pace — it does not teleport (owner decision, ADR-013). The two voids are absent from the list rather than offered and refused
  ...
  - the readout announces exactly twice per successful walk — `Walking to R11/KIT · Kitchen`, then the room label on arrival — and reports `Cannot walk to …` when there is no route or `Stopped before reaching …` when the body wedges; a walk you cancel yourself is silent
...
- Minimap (interior view only, `sm` and wider): the floor in plan at one SVG unit per metre, the room you are in filled amber, and an arrow showing your heading, sampled every 100 ms and written straight to the DOM so movement re-renders nothing. Its rooms are **pointer-selectable** — click one to walk there — and deliberately not focus stops: a room is about 15 px across, below the 24 px of WCAG 2.5.8, so "Go to room" is the full-size equivalent control for keyboard and touch (ADR-013). Screen readers get the information rather than the picture: "Floor minimap. You are in `R11/KIT · Kitchen`, facing toward side D"
...
- `Tab` from the view moves focus to the "Interior view" toggle, then to the "Third person" toggle, then to "Go to room" (and to "Stop walking" while a walk runs), then through the eight remote-control buttons; in the exterior view it is the "Interior view" toggle then the six camera-pad buttons. The room readout and the minimap are read, not operated, so neither is a tab stop. `Enter` on "Interior view" switches view, `Enter` or `Space` on "Third person" switches the camera mode and keeps focus on the button: `Shift+Tab` returns to the view (twice from "Third person", once from "Interior view")
- clicking "Third person" with the mouse switches the camera mode and returns focus to the view, so the keys keep working

## Test

- `pnpm verify:plan` — the source-of-truth verifier over `src/features/building/domain/sourceOfTruth/plan.ts`: 12 checks across 22 spaces, 96 derived walls, 20 ports, 9 windows and 7 fixtures. It prints the room table and the four totals closing on the plot, both dimension chains, the opening and matricule checks, reachability, the stair fit, wall thickness per contact, the fixture door-swing check, the isolation register, the walls built below storey height and the isolation-width check that nothing reads heavy while being built thin, then `PASS — 12 checks, 96 walls, 29 openings, 7 fixtures, everything closes` (verified). Run it first after touching the plan: it exits non-zero on the first broken invariant, and it is the first command of the DoD gate (ADR-010)
...
- `pnpm verify:plan | grep exact` — just the four totals: `floor 163.5150 · void 9.3600 · stairwell 8.0000 · walls 44.1250` (verified). The floor the app builds from them is FLOOR (slabs) 165.515 m² + VOID 9.360 + WALLS 44.125 + SHAFT 6.000 = 225.000 m² of plot, because only the 1.00 × 2.00 arrival landing of the 8.00 m² stair bay is floor at this level
- `pnpm test` — all unit tests, 1684 of them across 50 files (Vitest; jsdom for `src/`, node for `tooling/`) (verified)
- `pnpm test src/features/building/domain/viewMode.test.ts` — one file
- `pnpm test src/features/building/domain/walls.test.ts` — one file, the wall generator (the footprint it has to reproduce is the plan's WALLS total, 44.125 m², which `pnpm verify:plan` prints; 64 tests, green — it was one of the ~20 test files the plan-v2 migration reopened, and that migration is finished). This file is also the **only** check on the wall derivation's outline walk, clockwise ordering, matricule numbering and isolation/junction reasons — see the `pnpm verify:plan` entry above for why the verifier cannot catch a defect in any of them
...
- `pnpm verify:plan && pnpm typecheck && pnpm lint && pnpm format:check && pnpm test && pnpm build && pnpm test:e2e` — full gate (the plan is verified first: if the source of truth does not close, nothing downstream of it is worth running)
...
- `DEV_SERVER_PORT=5199 pnpm dev`, open http://localhost:5199 (5173 is taken on this machine) — walk the **whole floor** (22.50 × 10.00 m, ADR-008):
  ...
  - windows are 9 declared openings, and none of them sits in the side-C or side-D envelope: one from the control center onto the side-A balcony, one 0.65 m food tunnel from the guest room through to the kitchen, and seven facing the two side-B voids. They are deliberately not one size — bathroom ventilation slots above eye level (sill 1.90 m, head 2.30 m), the food pass at counter height (1.00–1.80 m), daylight windows at 0.90–2.10 m, and the laundry's single wide opening at 0.60–2.30 m. The living room and both kids bedrooms still get no daylight at all, and the master bedroom deliberately has no window
  ...
  - the interior opens on the stair arrival landing (5.10, 5.00) facing the corridor, never in a corner, and it opens in **first person**; `V` pressed there shows the person from close above and behind, because the 1.00 m landing leaves the follow camera only **0.500 m at 45.6° elevation** — exactly the 0.50 m floor below which the mannequin is hidden — so walk a couple of steps into the corridor and `V` there shows the whole person at the full 2.50 m follow distance (ADR-008, ADR-013)
