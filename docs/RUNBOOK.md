...
- `pnpm verify:plan` — the source-of-truth verifier over `src/features/building/domain/sourceOfTruth/plan.ts`: 12 checks across 21 spaces, 92 derived walls, 19 ports, 8 windows and 42 fixtures. It prints the room table and the four totals closing on the plot, both dimension chains, the opening and matricule checks, reachability, the stair fit, wall thickness per contact, the fixture door-swing check, the isolation register, the walls built below storey height and the isolation-width check that nothing reads heavy while being built thin, then `PASS — 12 checks, 92 walls, 27 openings, 42 fixtures, everything closes` (verified 2026-09-19). Run it first after touching the plan: it exits non-zero on the first broken invariant, and it is the first command of the DoD gate (ADR-010)
